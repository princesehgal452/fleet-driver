...
import MyLoads from 'assets/images/png/myLoads/MyLoads.png';
import MyLoadsActiveTracking from 'assets/images/png/myLoads/MyLoadsActiveTracking.png';
...
const useStyles = makeStyles((theme: ITheme) => ({
  ...
  actionButtonRoot: {
    color: theme.palette.bottomAppBarText,
  },
  actionButtonLabel: {
...
const DriverBottomBar = inject('driverAppStore')(observer(({ selected, onChange, driverAppStore }: IProps) => {
  ...
  const {
    userStore: {
      ...
      dispatchableDriver,
      FOUser: { permissions },
      defaultDriverCompletedOnboarding,
      dispatcherCompletedOnboarding,
      trackedMatchID,
    },
    configStore: { isV3 },
  } = driverAppStore as DriverAppStore;
  
  const allowSearchLoads = (defaultDriver || dispatcher || (dispatchableDriver && permissions?.allowSearch));
      ...
      >
        ...
        {(defaultDriver || dispatcher || (dispatchableDriver && permissions?.allowSearch)) && (
          <BottomNavigationAction
            label='My Loads'
            value='view'
            icon={<img src={trackedMatchID ? MyLoadsActiveTracking : MyLoads} alt='My Loads' />}
            classes={{
              root: classes.actionButtonRoot,
              label: classes.actionButtonLabel,
              selected: classes.actionButtonSelected,
